...
const firebaseConfig = {
  ...
  async function cargarDatosPreview() {
    ...
    if (!docId) {
      mostrarError("No se proporcionó un ID de documento válido en la URL.");
      return;
    }
      ...
      if (docSnap.exists) {
        ...
        const processedData = {
          ...
          fechaCreacion: docData.createdAt ? 
            ...
            'No disponible',
          fechaActualizacion: docData.updatedAt ? 
            docData.updatedAt.toDate().toLocaleString('es-MX') : 
            'No disponible'
        };
    ...
    const html = `
      <div class="preview-section">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 1.5rem; border-radius: 12px; margin-bottom: 2rem;">
          ...
          <p style="margin: 0; opacity: 0.9;">Fecha de creación: ${data.fechaCreacion}</p>
        </div>
      </div>
      ...
      <div class="preview-section">
        ...
        ${
          costos.length > 0
            ...
            `
            : "<div class='info-content'>No se registraron costos.</div>"
        }
      </div>
          ...
          ${imagenes.map((url, index) => `
            <div class="thumb">
              <a href="${url}" target="_blank" rel="noopener noreferrer">
                <img src="${url}" alt="Imagen clínica ${index + 1}" 
                     onerror="this.onerror=null; this.src='data:image/svg+xml;base64,'">
              </a>
            </div>
          `).join('')}
        </div>
      ...
      <div class="preview-section firmas">
        <div>
          <div style="border-top: 2px solid #333; width: 250px; margin: 20px 0;"></div>
      ...
      <div class="aviso-privacidad">
        ...
        <p>Sus datos personales serán utilizados únicamente con fines clínicos y administrativos conforme a la Ley Federal de Protección de Datos Personales en Posesión de los Particulares.</p>
      </div>
    `;
